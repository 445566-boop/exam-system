...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < questionBlocks.length; i += BATCH_SIZE) {
      ...
      const prompt = `你是题目提取助手。从文本中提取题目，输出严格的JSON数组格式。

【题型映射】单选题→单选，多选题→多选，判断题→判断，填空题→填空，简答题→简答

【输出格式】每题一个对象，字段：
- q: 题目内容（不含选项）
- a: 答案（选择题填选项字母如"A"，判断题填"正确"或"错误"，填空题填答案文本）
- t: 题型（单选/多选/判断/填空/简答）
- d: 难度（1-3，默认1）
...
【示例输出】
[{"q":"地球形状？","a":"B","t":"单选","d":1,"o":["A.方形","B.圆形"]}]
